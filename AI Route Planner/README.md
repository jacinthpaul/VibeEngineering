...
The LLM provider is **already wired to the real Claude API**. Click **Setup AI**
in the top bar and paste an Anthropic API key (from
[console.anthropic.com](https://console.anthropic.com/settings/keys)). The key is
stored in the browser (`localStorage`) and sent per-request to `/api/plan` via the
`x-anthropic-key` header — never persisted server-side. You can also set
`ANTHROPIC_API_KEY` in the environment as a fallback.

- With a key: the travel brief + blog intelligence are generated by
  `claude-opus-4-8` (`lib/providers/claude/llm.ts`, structured JSON output). The
  results header shows "Narratives generated live by Claude".
- Without a key (or on failure): the pipeline falls back to the deterministic
  mock narrative and surfaces the reason (`aiError`) in the UI. Maps, routes and
  weather remain mock for now.
...
| LLM text           | Templated narrative (fallback)              | **Claude API — live now**  |

Mock output is **deterministic** (seeded by the request), so demos are stable.

## Theme

Dark "Road Thrill" theme — black background with red accents, defined in
`app/globals.css` (CSS variables + a subtle red glow) and applied via Tailwind
utility classes throughout the components.

## Not yet built (next steps)

- Interactive map view of the route + stops
- Real Google Maps / weather provider implementations (LLM is already live)
